...
| --------------------- | ------------------------------------------ | --------------------------------- | --------------------------------------------- | ------------------------------------- |
| Markdown rendering    | `render-markdown.nvim` + old TS query path | likely yes, with reduced coupling | restore later; current markdown opens but no rendered headers/checkboxes | open markdown with fenced code blocks |
...
| Parser install        | `tree-sitter-manager.nvim`                 | `nvim-treesitter` install/update commands | remove `tree-sitter-manager.nvim` from active config | parser available for key filetypes    |
| Commentstring         | `nvim-ts-context-commentstring`            | maybe no direct core replacement  | restore for JSX/TSX through `Comment.nvim` pre-hook | comment TSX/JSX/HTML blocks           |
...
| Textobjects           | `nvim-treesitter-textobjects`              | maybe no core replacement         | intentionally drop; not used                  | none unless reintroduced later        |
| Incremental selection | `nvim-treesitter` module                   | maybe no core replacement         | intentionally drop; not used                  | none unless reintroduced later        |
| Illuminate references | `vim-illuminate` TS provider               | maybe LSP+regex only              | temporary replace with `lsp` + `regex`        | cursor on symbol                      |
| Embedded languages    | `otter.nvim` + TS                          | unclear                           | defer until after markdown work, then install/configure properly | test real target buffer after setup |
| Neotest parsing       | `neotest` + TS dep                         | unclear                           | keep dependency until adapter test proves otherwise | run nearest/file test            |
...
- [x] **Reintroduce parser management only if the native base still needs external install workflow**
  - Keep `tree-sitter-manager.nvim` only if it still adds real value.
  - 2026-07-13 final decision: removed `Spec("plugins.tree-sitter-manager")` from `lua/plugins.lua` and deleted `lua/plugins/tree-sitter-manager.lua`.
  - Reason: current pinned `nvim-treesitter` already provides parser install/update commands, and the extra manager no longer justified the added moving part.

- [x] **Reintroduce commentstring support if mixed-language commenting is lost**
  - Test TSX, JSX, HTML, markdown code regions if relevant.
  - 2026-07-13 intermediate step: removed the `ts_context_commentstring` `pre_hook` from `lua/plugins/comment.lua` so `Comment.nvim` used its native filetype and `commentstring` resolution.
  - 2026-07-13 final decision: restored `nvim-ts-context-commentstring` specifically for JSX/TSX comments by adding it back as a `Comment.nvim` dependency and restoring `pre_hook = require("ts_context_commentstring.integrations.comment_nvim").create_pre_hook()`.
  - Reason: upstream `Comment.nvim` docs explicitly call out `jsx/tsx` as a case that still needs the external integration.
...
- [ ] **Reintroduce textobjects and incremental selection only if they still work reliably**
  ...
  - 2026-07-13 note: the old `nvim-treesitter-textobjects` and incremental-selection config is no longer present in the active `lua/plugins/treesitter.lua` base. Only the preserved reference snippet in this plan still mentions it.
  - 2026-07-13 decision: do not reintroduce them unless a real use case appears; user does not use them.
...
- [ ] **Audit `neotest` and `otter.nvim` dependencies after the base migration**
  ...
  - 2026-07-13 finding: upstream `neotest` README still documents `nvim-treesitter` as a dependency and `neotest-vitest` says Treesitter plus the matching language parser must be installed.
  - 2026-07-13 finding: upstream `otter.nvim` README explicitly lists `nvim-treesitter/nvim-treesitter` as a dependency for injected-language support.
  - 2026-07-13 decision: keep `neotest` as-is for now because it appears to work.
  - 2026-07-13 decision: defer `otter.nvim` work until after markdown rendering is fixed, then install/configure it properly against the new Tree-sitter base.
  - 2026-07-13 note: user does not currently use `otter.nvim`, so this remains a later install/configure task rather than part of the stabilization path.
...
- [ ] **Remove obsolete compatibility hacks only after the new stack is proven**
  ...
  - 2026-07-13 result: removed the extmark monkey-patch from `lua/autocmd.lua` and re-verified clean headless startup plus markdown and Lua buffer opens. No suppressed `Invalid 'col': out of range` path was needed after the migration cleanup.
...
- [ ] **Write final migration notes with old vs new behavior**
  ...
  - Include a follow-up item to restore markdown rendering features such as rendered headers and checkboxes once a safe Neovim 0.12-compatible path is found.
  - Include a follow-up item to install and configure `otter.nvim` properly after markdown rendering is stable.
