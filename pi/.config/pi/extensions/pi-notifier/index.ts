...
const PLAYERS = [
	{
		cmd: "afplay",
		args: (path: string, volume: number) => ["-v", `${volume}`, path],
	},
	{
		cmd: "paplay",
		args: (path: string, volume: number) => [`--volume=${Math.round(volume * 65536)}`, path],
	},
	{ cmd: "aplay", args: (path: string) => [path] },
	{
		cmd: "mpv",
		args: (path: string, volume: number) => [`--volume=${Math.round(volume * 100)}`, "--no-terminal", path],
	},
	{
		cmd: "ffplay",
		args: (path: string, volume: number) => ["-nodisp", "-autoexit", "-volume", `${Math.round(volume * 100)}`, path],
	},
]
	...
	if (process.env.TMUX) {
		try {
			const out = execFileSync(
				"tmux",
				["display-message", "-p", "#{session_attached} #{window_active} #{pane_active}"],
				{
					encoding: "utf8",
					timeout: 1000,
				},
...
function playSound(event: string) {
	...
	playWithFallback(path, volume)
}

function playWithFallback(path: string, volume: number, playerIndex = 0) {
	const player = PLAYERS[playerIndex]
	if (!player) return

	execFile(player.cmd, player.args(path, volume), { timeout: 30000 }, (error) => {
		if (!error) return
		playWithFallback(path, volume, playerIndex + 1)
	})
}
