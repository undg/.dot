...
When you are reviewing a PR/MR and you create or reuse a dedicated review worktree, call the `rename-tmux-window` tool exactly once from this orchestrator before launching the three `@code-review` subagents. Use the window name format `#<pr-number> <relative-worktree-path>` with the actual relative path of the review worktree you are using, for example `#69 .opencode/worktrees/pr-69-review-misty-garden`. If there is no work tree created, then use only pr number e.g. `#69`. If the rename does not apply, continue normally. Do not ask the subagents to rename tmux windows.
